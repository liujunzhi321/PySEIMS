...
class Parameters(object):
    ...
    acm_reflex = np.array([0.0156935, 4.22273, 208.868, 0.0453194,
                                   ...
                                   0.789798])
    acm_constants = acm_reflex  # (currently using params from REFLEX)
    
def acm(cf, clma, ceff, t_max, t_range, i, ca, julian_day, lat, acmc):
    ...
    if s >= 24.:
        ...
        s = s
    gpp = (i*gc*(ca - ci))*(acmc[0]*s + acmc[3]) / (E0*i + gc*(ca - ci))
    return gpp
...
def run(inital_state, julian_day, t_mean, t_max, t_min, i, ca, lat, paraList, water_stress=1):
    """
    ...
    theta_t,d_onset,d_fall,c_eff,c_lma,c_lf,c_ronset,c_rfall,acm_constants = paraList[0:17]
    
    gpp = acm(cfol, c_lma, c_eff, t_max, t_max-t_min, i, ca, julian_day, lat, acm_constants)*water_stress
