...
def acm(self, cf, clma, ceff, tmax, t_range, i, ca, julian_day, lat, constants):
...
def run(inital_state, julian_day, t_mean, t_max, tmin, i, ca, lat, water_stress=1, paraList):
    """
        inital_state, 六个碳库的初始状态
        julian_day,   一年中的第几天
        soil_moisture,土壤含水量
    ...
    gpp = ACM(cf, clma, ceff, tmax, t_range, i, ca, julian_day, lat, acm_constants)*water_stress
    phi_onset = onset(julian_day, d_onset, c_ronset)
    phi_fall  = fall(julian_day, d_fal, c_rfall, 1/c_lf)
    temp = np.exp(t_mean * theta_t)
