...
class DALEC(object):
    ...
    def set_value(self, k, val):
        if not k in self.alias.keys():
            ...
            return -1
        setattr(self, self.alias[key], val)
        
    def get_value(self, k):
        return getattr(self, self.alias[key], None)
    ...
    def run(self, inital_state, julian_day, t_mean, t_max, t_min, i, ca, lat, water_stress=1):
        ...
        cfol_next = (1 - phi_fall)  * cfol + (1 - self.autof) * self.folf * gpp + phi_on * clab
